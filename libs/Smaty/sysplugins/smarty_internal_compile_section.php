...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
     ...
     * @var array
     */
    public $nameProperties = array('first', 'last', 'index.class', 'iteration', 'show', 'total', 'rownum', 'index_prev',
                                   'index_next', 'loop');
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $incFor = array();
        $cmpFor = array();
        $propValue = array('index.class' => "{$sectionVar}->value['index.class']", 'show' => 'true', 'step' => 1,
                           'iteration' => "{$local}iteration",

        );
        $propType = array('index.class' => 2, 'iteration' => 2, 'show' => 0, 'step' => 0,);
        ...
        if ($propType[ 'step' ] == 0) {
            if ($propValue[ 'step' ] == 1) {
                $incFor['index.class'] = "{$sectionVar}->value['index.class']++";
            } elseif ($propValue[ 'step' ] > 1) {
                $incFor['index.class'] = "{$sectionVar}->value['index.class'] += {$propValue['step']}";
            } else {
                $incFor['index.class'] = "{$sectionVar}->value['index.class'] -= " . - $propValue[ 'step' ];
            }
        } else {
        ...
        $initFor['index.class'] = "{$sectionVar}->value['index.class'] = {$propValue['start']}";
        ...
        if (isset($namedAttr[ 'index_prev' ])) {
            $output .= "{$sectionVar}->value['index_prev'] = {$propValue['index.class']} - {$propValue['step']};\n";
        }
        if (isset($namedAttr[ 'index_next' ])) {
            $output .= "{$sectionVar}->value['index_next'] = {$propValue['index.class']} + {$propValue['step']};\n";
        }
